...
[ ! -d ".qwen/epics" ] && echo "📁 No epics directory found. Create your first epic with: /pm:prd-parse <feature-name>" && exit 0
[ -z "$(ls -d .qwen/epics/*/ 2>/dev/null)" ] && echo "📁 No epics found. Create your first epic with: /pm:prd-parse <feature-name>" && exit 0
...
echo ""
echo "📊 Summary"
total=$(ls -d .qwen/epics/*/ 2>/dev/null | wc -l)
tasks=$(find .qwen/epics -name "[0-9]*.md" 2>/dev/null | wc -l)
